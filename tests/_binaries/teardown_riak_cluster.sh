...
riak_leave_plan () {
    HEAD_NODE=$1
    LEAVING_NODE=$2
    if [ "$HEAD_NODE" = "$LEAVING_NODE" ]; then
        return 0
    fi
    echo "leaving $LEAVING_NODE to $HEAD_NODE"
    $TMP_BASE$LEAVING_NODE/bin/riak-admin cluster leave
}
riak_leave_commit () {
    HEAD_NODE=$1
...
for node_name in $NODE_NAMES; do
    ...
    riak_leave_plan $HEAD_NODE $node_name
done
riak_leave_commit $HEAD_NODE
